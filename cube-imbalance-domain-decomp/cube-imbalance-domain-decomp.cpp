...
int main(int argc, char *argv[]) {
  ...
  int slice_size = (*cfg)["sim"]["tslice"].read<int>();
  hlog << "Slice time : " << slice_size << endl;
...
//  hemocell.setSystemPeriodicity(0, true);
  ...
  while (hemocell.iter < tmax) {
    ...
    if(hemocell.iter % slice_size == 0 && hemocell.iter != 0) {
  ...
  std::fstream sout;
  string filename = plb::global::directories().getLogOutDir() + "neighbours.neighbours";
  ...
  */
  ...
  return 0;
	...
	/*
        for(int tmpi = 0 ; tmpi < (int)overlapsP.size(); tmpi++){
                bool self = false;

                for (int tmpj = 0; tmpj < (int) hemocell.lattice->getMultiBlockManagement().getLocalInfo().getBlocks().size(); tmpj++) {
                        if (overlapsP[tmpi].overlap.getOriginalId() == hemocell.lattice->getMultiBlockManagement().getLocalInfo().getBlocks()[tmpj]){
                                self = true;
                                break;
                        }
                }

                if (self) sout << " " << overlapsP[tmpi].overlap.getOverlapId();
        }
	*/
